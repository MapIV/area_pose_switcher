...
{
public:
    PoseSwitcher() : Node("pose_switcher"), is_gnss_pose_received_(false)
    {
        // Parameters
        ...
        // yaw threshold: deg -> rad
        error_yaw_threshold_ *= M_PI / 180.0;
        ...
        // Initialization of shared_ptr placeholders
...
private:
    ...
    bool computeError(Error2dYaw& error_2d_yaw) {
        ...
        geometry_msgs::msg::Pose gnss_pose  = gnss_pose_msg_ptr_->pose.pose;
        geometry_msgs::msg::Pose lidar_pose = interpolator.get_current_pose().pose.pose;

        // 2D error
        double error_2d = std::hypot(gnss_pose.position.x - lidar_pose.position.x,
                                     gnss_pose.position.y - lidar_pose.position.y);
        // yaw error (rad)
        ...
        error_2d_msg.data  = static_cast<float>(error_2d);
        error_2d_pub_->publish(error_2d_msg);

        tier4_debug_msgs::msg::Float32Stamped error_yaw_msg;
        error_yaw_msg.stamp = sensor_ros_time;
        error_yaw_msg.data  = static_cast<float>(error_yaw * 180.0 / M_PI); // [deg] for readability
    ...
    std::deque<geometry_msgs::msg::PoseWithCovarianceStamped::ConstSharedPtr> gnss_pose_cov_msg_ptr_array_;
    std::deque<geometry_msgs::msg::PoseWithCovarianceStamped::ConstSharedPtr> lidar_pose_cov_msg_ptr_array_;

    double error_2d_threshold_;
    double error_yaw_threshold_; // [rad]
    double judge_switching_ealpsed_time_threshold_;
    double judge_switching_first_time_ = -1.0;

    map4_localization_msgs::msg::LocalizationTypeStamped current_localization_type_msg_;
    map4_localization_msgs::msg::LocalizationTypeStamped area_localization_type_msg_;
